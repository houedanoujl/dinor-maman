<button wire:click="toggleVote"
        ...
        @disabled($hasVoted || $hasVotedAnywhere || $contestEnded)
        ...
        type="button"
        title="{{ $hasVoted ? 'Vous avez voté pour cette photo.' : ($hasVotedAnywhere ? 'Vous avez déjà voté pour un autre participant.' : 'Cliquer ici pour voter') }}"
        aria-label="{{ $hasVoted ? 'Vous avez voté' : ($hasVotedAnywhere ? 'Vote déjà utilisé' : 'Voter pour ' . $participant->full_name) }}">
    ...
    @if (! $hasVoted && ! $hasVotedAnywhere && ! $contestEnded)
        <span class="font-semibold">· Cliquez pour voter</span>
    @elseif ($hasVoted)
        <span class="font-semibold">· Voté ✓</span>
    @elseif ($hasVotedAnywhere)
        <span class="font-semibold">· Déjà voté</span>
    @endif
</button>
